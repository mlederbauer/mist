#!/bin/bash
#SBATCH --job-name=nist23_fp_mist_mhplus_aux_gate_all
...
# Same as submit_nist23_fp_mist_mhplus_aux_gate.sh, plus the new
# reaction_metadata_suong.tsv (CAS name-reaction extraction, 55,360 rows) added
# to the --reaction-metadata-file list -- all 4 sources (uspto, cas, pistachio,
# suong) now feed the same starting_materials/candidates aux-gate mechanism.
# [M+H]+-only (data/nist23/labels_mh_only.tsv), same hyperparameters as every
# other aux_gate variant in notebooks/gate_results_summary.ipynb.
pixi run python src/mist/train_mist.py \
    ...
    --save-dir results/nist23_fp_mist_mhplus_aux_gate_all/split_1 &
