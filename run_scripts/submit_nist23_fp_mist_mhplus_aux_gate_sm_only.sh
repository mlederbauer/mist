#!/bin/bash
#SBATCH --job-name=nist23_fp_mist_mhplus_aux_gate_sm_only
...
# Ablation variant of submit_nist23_fp_mist_mhplus_aux_gate.sh (the "both
# sources" run): restricts --aux-gate to starting_materials only via
# --aux-sources, dropping candidates entirely (no gate head/projection built
# for it, no data loaded for it). Compare against the "both" and
# "candidates only" variants to see which source (if either) the gate
# actually benefits from -- starting_materials has much higher coverage
# (~48% of NIST23 compounds) but slightly lower per-compound Tanimoto
# correlation with the true product (~0.31) than candidates (~0.34, but
# only ~8% coverage) per notebooks/reaction_aux_fp_correlation.ipynb.
pixi run python src/mist/train_mist.py \
    ...
    --no-diffs \
    --aux-gate \
    --aux-sources starting_materials \
    --aux-dropout 0.2 \
    ...
    --save-dir results/nist23_fp_mist_mhplus_aux_gate_sm_only/split_1 &
