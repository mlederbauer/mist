#!/bin/bash
#SBATCH --job-name=nist23_fp_mist_mhplus_aux_gate_cand_only
...
# sources" run): restricts --aux-gate to candidates only via --aux-sources,
# dropping starting_materials entirely. Compare against the "both" and
# "starting_materials only" variants. candidates has much lower coverage
# (~8% of NIST23 compounds) but a somewhat higher per-compound Tanimoto
# correlation with the true product (~0.34) than starting_materials
# (~0.31, ~48% coverage) per notebooks/reaction_aux_fp_correlation.ipynb --
# this run isolates whether that stronger-but-rarer signal is worth using
# on its own.
pixi run python src/mist/train_mist.py \
    ...
    --save-dir results/nist23_fp_mist_mhplus_aux_gate_cand_only/split_1 &
